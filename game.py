...
class MemoryGame:
    ...
    def create_difficulty_selection(self) -> None:
        ...
        label = tk.Label(self.root, text="Select Difficulty Level:", font=("Helvetica", 14))
        label.grid(row=0, column=0, columnspan=3)

        for idx, (level, (rows, cols)) in enumerate(self.difficulty_options.items()):
            button = tk.Button(self.root, text=level, command=lambda l=level: self.start_game(l))
            button.grid(row=1, column=idx)
    ...
    def create_widgets(self, rows: int, cols: int) -> None:
        ...
        for row_index in range(rows):
            ...
            for column_index in range(cols):
                ...
                button.grid(row=row_index + 2, column=column_index)  # Offset by 2 for difficulty selection
                row.append(button)
            self.buttons.append(row)

        # Timer label
        self.timer_label = tk.Label(self.root, text="Time Remaining: 60s", font=("Helvetica", 14))
        self.timer_label.grid(row=rows + 2, column=0, columnspan=cols)
